...
*signals* are not uniformly dead on disclosure. A signal survives publication
...
- **Omission** — withhold an oracle the analyzer only checks when present
  (worker-scope ≥2 guard; **missing `device.mac`**). *Absence treated as clean.*
...
`Page.addScriptToEvaluateOnNewDocument`) runs in *every* document realm
...
Current SDKs normally run the JSVM + crypto/submission bridge inside a dedicated
Worker. But if Worker creation/fetch/start fails, the iframe launcher still has
a legacy fallback path that calls `runArgusVm()` in the iframe. A smart attacker
can intentionally break `Worker`, `Blob`, `URL.createObjectURL`, worker fetch,
or CSP so the SDK falls back into exactly the realm Playwright
`addInitScript` can pre-poison. Once downgraded, the nested pristine iframe is
again a speed bump rather than a boundary, because `addInitScript` runs in every
new document realm.
...
- **Server-authoritative hydration.** `tcp_probe`/`h2` are deleted from client
  ...
  not fallback to attacker data. Verification failure *removes authority*.
...
### MP-1 — The worker is the *sole* holder of submission capability
...
replays it. *Any* Class-A field forgery fails in one check.
...
Status: `device.mac` chain shipped 2026-05-26 — but see Gap 1 (absence tolerated).
...
The deepest shift and the one that *enables* the open-source goal. As long as the
...
| device-identity-mint-and-replay (v1) | A/replay | MP-4 payload-bound sig | CLOSED (v2) |
| **direct-wire-mac-absence-regression** | **A/omission** | **MP-2 enforcement (`absent`→400, `worker_attest` mandatory)** | **CLOSED 2026-06-09 — API rejects no-MAC at ingestion (HTTP 400 `device_mac_required`); bot `--expect fixed` passes, `fixedAtBoundary: true`. Clean current-SDK run still 200 (no regression).** |
...
> **✅ STATUS 2026-06-09: Gaps 1 + 2 + 2b CLOSED (clean break shipped).** SDK
...
### Gap 1 (P0) — `device.mac` absence is tolerated; live regression proves ingestion acceptance
**Verified in code 2026-06-08.** `verifyDeviceMac()`
...
payloads *without* `worker_attest`.
...
no-MAC payload. `identification.verified` is an identity-*resolution* flag, NOT a
...
Fix (see the consolidated rule at the top of this section): make `absent` a hard
reject and mandate an authentic `worker_attest`-bearing `device.mac`. No
production users → ship it as a clean break (delete the legacy 22-slice fallback),
no version gate or cutover needed.

### Gap 2 — compatibility fallbacks become permanent bypass classes
The MAC verifier accepts the legacy slice order without `worker_attest` during
rollout. Reasonable — but every compat fallback needs an **owner, removal date,
metric threshold, and a test proving new bundles no longer need it.** Otherwise
"support old clients" becomes "bots choose the old protocol." The rollout/legacy
seam IS an attack surface.

### Gap 2b — Worker failure can downgrade into a poisoned iframe VM
The iframe launcher still has a fallback: if `runViaWorker()` fails, it calls
...
Caveat the "no submission" branch: if Worker-failure yields *nothing*, an attacker
...
`device_identity` v2 is payload-bound (good for continuity) but a Node script
...
1. **No authoritative field without provenance.** Every load-bearing field has a
   ...
   `server_verified` feeds clean verdicts. Failed verification and *expected*
...
3. **Tampered objects cannot expose trusted accessors.** Server-side
   ...
   type Rejected = { ok: false; reason: string; tampered?: boolean; expired?: boolean };
...
- **P0 — Close the legacy MAC/`worker_attest` window (Gaps 1 + 2 + 2b, one clean
  break).** Not in production → no migration. Server: require an authentic
