...
/**
 * Maximum evasion bot — vanilla Puppeteer + sticky proxy + TZ spoof:
 * - No stealth plugin (avoids lie detection)
 * - SOAX sticky session: same IP across all probes
 * - Timezone spoofed via CDP to match proxy exit country
...
const STICKY_SESSION = 'package-267218-country-ie-sessionid-evasiontest789-sessionlength-300';
...
test.describe('adversarial: maximum evasion bot', () => {
  test.skip(!SOAX_PASS || !!process.env.SKIP_PROXY, 'PROXY_PASSWORD not set or SKIP_PROXY=1');

  test('puppeteer + sticky Irish proxy + TZ spoof + fake WebRTC IP', async ({ request }) => {
    ...
    try {
      ...
      // Spoof timezone to match Ireland exit node
