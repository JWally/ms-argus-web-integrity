...
const AUTOMATION_GLOBALS: [string, () => unknown][] = [
  // Modern frameworks (Playwright / Puppeteer / cdc_) — argus's existing list.
  ...
  // Historic / long-tail bot frameworks — mirrors FingerprintJS slot
  // s157. Each is a single property read; aggregate cost is trivial.
  // Catches the framework tail that modern lists miss.
  ['awesomium', () => (window as any).awesomium],
  ['cef', () => (window as any).cef],
  ['cefsharp', () => (window as any).CefSharp],
  ['coachjs', () => (window as any).coachjs],
  ['fminer', () => (window as any).fminer],
  [
    'geb',
    () => (window as any).geb || (window as any).__$webdriverAsyncExecutor,
  ],
  [
    'nightmarejs',
    () => (window as any).Nightmare || (window as any).__nightmare,
  ],
  ['rhino', () => (window as any).Packages || (window as any).java],
  [
    'selenium',
    () => (window as any).selenium || (window as any).Selenium_IDE_Recorder,
  ],
  ['webdriver', () => (window as any).webdriver],
  ['domAutomation', () => (window as any).domAutomation],
  ['domAutomationController', () => (window as any).domAutomationController],

  // Agentic browser tooling (2024-2026 wave per DataDome's denylist).
  // EXPLICIT EXCLUSION: do not detect Claude Code. We intentionally
  // skip its known globals.
  ['stagehand_v3', () => (window as any).__stagehandV3__],
  ['genspark', () => (window as any).genspark],
  ['browser_use', () => (window as any).__browser_use__],
  ['fellou', () => (window as any).__fellou__],
  ['perplexity_browser', () => (window as any).__perplexity_browser__],
];
...
/**
 * isTrusted event probe (mirrors FingerprintJS slot s165).
 *
 * Real user-initiated events have `event.isTrusted === true`. Events
 * generated by `dispatchEvent` from script have `isTrusted === false`.
 * Some automation frameworks patch this incorrectly:
 *   - Puppeteer's `page.click()` traditionally yields isTrusted=false
 *     unless the host uses CDP's Input.dispatchMouseEvent (which
 *     yields true). A patched Event constructor that sets isTrusted
 *     to true is a tell.
 *   - Naive stealth scripts try to spoof isTrusted via Object.defineProperty
 *     on the prototype, which fails because isTrusted is a non-config
 *     accessor on Event.prototype.
 *
 * We dispatch a synthetic event and report the observed isTrusted. A
 * legitimate browser ALWAYS returns false for script-dispatched events;
 * a `true` return = automation tooling that's lying about isTrusted.
 */
function probeIsTrusted(): { value: boolean | null; threw: boolean } {
  try {
    let observed: boolean | null = null;
    const target = document.createElement('div');
    const handler = (e: Event) => {
      observed = e.isTrusted;
    };
    target.addEventListener('argus-isTrusted-probe', handler, { once: true });
    target.dispatchEvent(new Event('argus-isTrusted-probe'));
    return { value: observed, threw: false };
  } catch {
