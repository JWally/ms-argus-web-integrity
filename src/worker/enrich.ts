 ...
 */
export function enrichScope(scope: {
  userAgent: string;
  ...
  const decrypted = decryptUserAgent({
    ua: userAgent,
    os: system,
    isBrave: false,
  });
  ...
  return {
    ...
    userAgentDataVersion: extractVersion(
      scope.userAgentData
        ? (scope.userAgentData.uaFullVersion as string) || ''
        : '',
    ),
    userAgentEngine:
      /safari/i.test(decrypted) || /iphone|ipad/i.test(userAgent)
        ? 'JavaScriptCore'
